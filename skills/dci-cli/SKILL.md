...
- `--page-token` and `--max-results` are **rejected** on commands that return their whole response in one request (they declare no page token). `--all` is accepted everywhere and notes on stderr when there was nothing more to fetch.
- **File-shaped exports** (`export-datahub-dataset-records`) return CSV or, with `--format jsonl`, newline-delimited JSON — printed verbatim, so `--output` does not apply. Their page token arrives in the `X-Next-Page-Token` header and the API caps each request at 366 days; `--all` follows the tokens **and** walks the windows, merging CSV pages against the union of their headers. `--all` with no `--start-time`/`--end-time` exports the dataset's full range (the CLI probes backwards for the data's start, one row per window, and reports the range on stderr); both bounds are required otherwise. Use `--output-file <path>` (or `--output-file .` for the API's suggested filename) to save the bytes, and `--for-reimport` to rewrite an export for `ingest-datahub-events-csv`. `--search` does not apply to a file body and is rejected.
