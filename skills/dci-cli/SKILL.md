...
- **File-shaped exports** (`export-datahub-dataset-records`) return CSV or, with `--format jsonl`, newline-delimited JSON — printed verbatim, so `--output` does not apply. Their page token arrives in the `X-Next-Page-Token` header and the API caps each request at 366 days; `--all` follows the tokens **and** walks the windows, merging CSV pages against the union of their headers. Use `--output-file <path>` (or `--output-file .` for the API's suggested filename) to save the bytes, and `--for-reimport` to rewrite an export for `ingest-datahub-events-csv`.
