...
func addOutputFlag() {
	...
	dciCmd.PersistentFlags().String("chart", "", "Render a chart of the report under the table: 'stacked' columns per group (the default), a 'line' of period totals, a one-line 'sparkline', a group-by-period 'heatmap', or a 'treemap' of group shares (table output in human mode only; ignored elsewhere)")
	...
	dciCmd.PersistentFlags().Bool("all", false, "Fetch every page before rendering (follows the server's page tokens; GET list commands and file exports). On export-datahub-dataset-records it also walks the API's 366-day windows, and on its own — with no --start-time/--end-time — exports the dataset's full range. Cannot be combined with --page-token or --max-results")
	...
	dciCmd.PersistentPreRunE = func(cmd *cobra.Command, args []string) error {
		...
		if flag := cmd.Flags().Lookup("chart"); flag != nil && flag.Changed {
			...
			switch chartMode {
			case "stacked", "line", "sparkline", "heatmap", "treemap":
			case "true": // the flag's former boolean spelling keeps working
				chartMode = "stacked"
			case "false", "":
				chartMode = ""
			default:
				return fmt.Errorf("invalid --chart %q (supported: stacked, line, sparkline, heatmap, treemap)", flag.Value.String())
			}
		}
