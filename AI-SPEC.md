...
The session runs in the **alternate screen** with a stable frame: a one-line header, the transcript in a `bubbles/viewport`, and the input + status pinned at the bottom. The frame never scrolls; the transcript scrolls *inside* the viewport (mouse wheel, PgUp/PgDn, ctrl+u/ctrl+d), auto-following the bottom until the user scrolls up. The first draft specified inline scrollback here; dogfood preferred the stable frame (D9), accepting the trade: native terminal scrollback is exchanged for a frame that never jumps, with `/export` covering transcript retention and `/clear` resetting it. Mouse capture defaults **off** so the terminal's own text selection and copy work untouched (dogfood: capture broke copy/paste even with modifier keys in some terminals); `/mouse` opts into wheel scrolling for the transcript, and PgUp/PgDn always scroll regardless.

**Frame grid and repaint.** The rendered frame is exactly the terminal's cell grid — every line clamped to the width, exactly one line per row — and the session is redrawn in full by **ctrl+l**. Both follow from how Bubble Tea's standard renderer paints: it rewrites only lines whose text differs from the last frame, so anything that moves the frame without changing its content leaves the screen wrong indefinitely. The common case is the default mouse setting: with capture off the wheel belongs to the terminal, which in the alternate screen slides the frame out of view and composites main-buffer scrollback into the exposed rows; the banner, being the one block that never changes, is what stays smeared afterwards. A frame that owns every cell makes one repaint sufficient to restore all of it. Frame lines are trimmed with intent rather than at the right edge where the priority is known — the status line trims the narration snippet, never the elapsed time or the cancel affordance — and a pane too short for the chrome drops transcript rows from the top, keeping the input and status.
